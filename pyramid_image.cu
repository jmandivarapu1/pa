 ...
 * Pyramid of Images
 * Host code.
 * for creating blurring then creating
 * pyramid of images
 * 
 */
...
//	int num_blocks = (height * width + num_threads - 1) / num_threads;
 ...
 // printf("%d--%d--%d--%d--%d\n",i,j,k,(i * (width) * 3 + j * 3 + k),(i * (width) * 3 + (j+1) * 3 + k));
    ...
    free(result_mat);
    //char buf[12];
    //sprintf(buf, "pre_%d_suff", 2);
   // char dst[12] = "pre_";
   // strcat(dst, "_suff",atoi(1));
    //printf("%s\n",buf);
  for (int m=0;m<no_of_levels;m++){
    char buf[12];
    sprintf(buf, "cp test_4.png ./output/test_%d.png", m);
    printf("%s\n",buf);
    printf("%s\n","finished");
    system(buf);
    system("rm blurred_out.png");
    system("cp test_4.png ./blurred_out.png");
    system("./blur-effect.out ./blurred_out.png  9 256 652 0");
}
    return 0;
}
